...
vector<operation> apply_z_funtion(board cur_board)
{
    ...
    for (ll i=0; i<height; i++)
    {
        for (ll j=0; j<width; j++) 
        {
            ...
            for (ll l=len-1; l>=0; l--) if (y+l<width)
            {  
                ll tmp_x=x, d=tmp_x-i;
                assert(d>=0);
                for (ll k=8; k>=0; k--) if (d&(1LL<<k))
                {
                    cur_board=cur_board.apply_die(operation((k==0?0:3*k-2), tmp_x-(1LL<<k), y+l, TOP));
                    answer.push_back(operation((k==0?0:3*k-2), tmp_x-(1LL<<k), y+l, TOP));
                    // cout<<"[direction, id, x, y]:   TOP "<<dies[(k==0)?0:3*k-2].height<<" "<<tmp_x-(1LL<<k)<<" "<<y+l<<" "<<endl;
                    // cur_board.print();
                    tmp_x-=(1LL<<k);
                }
            }
