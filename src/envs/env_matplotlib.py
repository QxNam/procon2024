...
class BoardTransformEnv(gym.Env):
    def __init__(self, board, goal, dies=None):
        ...
        self.action_space = spaces.MultiDiscrete(
            [len(dies), self.width, self.height, 4])  # 4 directions

        # Define observation space: board state
        self.observation_space = spaces.Box(
            low=0, high=3, shape=(self.height, self.width), dtype=np.int32)
        self.reset()

        # Initialize rendering
        self.fig, self.ax = plt.subplots()
        self.im = None
        # Tùy chỉnh màu sắc cho các giá trị
    ...
    def step(self, action):
        ...
        # Negative reward based on differences
        reward = -np.sum(self.state != self.goal) - self.n_steps
        done = np.array_equal(self.state, self.goal)
        self.n_steps += 1
        return self.state, reward, done, self.n_steps

    def render(self):
        if self.im is None:
            self.im = self.ax.imshow(
                self.state, cmap=self.cmap, norm=self.norm)
