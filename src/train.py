...
for episode in range(num_episodes):
    ...
    for t in range(100):  # Limit steps per episode
        ...
        if replay_buffer.size() >= batch_size:
            ...
            states = states.view(states.size(0), -1)  # [32, 24]
            next_states = next_states.view(next_states.size(0), -1)
    
            # Compute Q-values for the actions taken
            q_values = model(states).gather(1, actions.view(-1, 1))  # [32, 1]
            q_values = q_values.squeeze(1)  # [32]
    
            # Compute the target Q-values (from target model)
            next_q_values = target_model(next_states).max(1)[0]  # [32]
            targets = rewards + gamma * next_q_values * (1 - dones)  # [32]
